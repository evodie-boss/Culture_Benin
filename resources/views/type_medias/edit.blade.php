...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
    <div class="flex items-center mb-4 sm:mb-0">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Modifier le Type de Média</h1>
        ...
        <ol class="inline-flex items-center space-x-2 text-sm">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('type-medias.index') }}" class="ml-1 text-gray-500 hover:text-blue-600">Types de Médias</a>
                </div>
            </li>
    ...
    <div class="bg-gradient-to-r from-blue-50 to-green-50 px-6 py-5 border-b border-gray-200">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center mb-4 sm:mb-0">
                ...
                <div>
                    <h2 class="text-xl font-bold text-gray-900">Modifier le type de média #{{ $typeMedia->id_type_media }}</h2>
                    <p class="text-gray-600 mt-2">Mettre à jour le nom et la description</p>
                </div>
            </div>
            <div class="bg-white rounded-xl px-4 py-2 shadow-md border">
                <span class="text-sm text-gray-600">Nom actuel: </span>
                <span class="font-semibold text-blue-600">{{ $typeMedia->nom_type_media ?? 'N/A' }}</span>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('type-medias.update', $typeMedia->id_type_media) }}" method="POST">
        ...
        <div class="p-6">
            ...
            <div class="space-y-3 mb-6">
                <label for="nom_type_media" class="block text-sm font-semibold text-gray-700">
                    Nom du type de média <span class="text-red-500">*</span>
                </label>
                <div class="relative">
                    <div class="absolute top-4 left-4 flex items-start pointer-events-none">
                        <i class="fas fa-font text-gray-400 text-lg mt-1"></i>
                    </div>
                    <input type="text" 
                           class="pl-12 w-full rounded-xl border border-gray-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('nom_type_media') border-red-500 @enderror" 
                           id="nom_type_media" 
                           name="nom_type_media" 
                           value="{{ old('nom_type_media', $typeMedia->nom_type_media) }}"
                ...
                @enderror
                <p class="text-gray-500 text-sm">Nom complet du type de média</p>
            </div>
            ...
            <div class="space-y-3 mb-6">
                ...
                <div class="relative">
                    ...
                    <textarea class="pl-12 w-full rounded-xl border border-gray-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('description') border-red-500 @enderror" 
                              ...
                              name="description" 
                              rows="5" 
                              placeholder="Décrivez ce type de média...">{{ old('description', $typeMedia->description) }}</textarea>
                </div>
                @error('description')
                    <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                @enderror
                <p class="text-gray-500 text-sm">Informations complémentaires sur le type de média</p>
            </div>

            <!-- Informations du type de média -->
            <div class="bg-blue-50 border-l-4 border-blue-500 rounded-xl p-5 mb-6">
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <h4 class="text-base font-semibold text-blue-900 mb-3">Informations du type de média</h4>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm text-blue-800">
                            <div class="flex items-center">
                                <i class="fas fa-hashtag mr-3 text-blue-600"></i>
                                <span><strong>ID :</strong> #{{ $typeMedia->id_type_media }}</span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-clock mr-3 text-blue-600"></i>
                                <span><strong>Modifié le :</strong> 
                                    {{ $typeMedia->updated_at ? $typeMedia->updated_at->format('d/m/Y H:i') : 'Jamais' }}
                                </span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-calendar-plus mr-3 text-blue-600"></i>
                                <span><strong>Créé le :</strong> 
                                    {{ $typeMedia->created_at ? $typeMedia->created_at->format('d/m/Y H:i') : 'N/A' }}
                                </span>
                            </div>
                            <div class="flex items-center">
                                <i class="fas fa-layer-group mr-3 text-blue-600"></i>
                                <span><strong>Type :</strong> 
                                    {{ $typeMedia->nom_type_media ?? 'N/A' }}
                                </span>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const initialValues = {
        nom_type_media: document.getElementById('nom_type_media').value,
    ...
    const cancelLink = document.querySelector('a[href="{{ route('type-medias.index') }}"]');
